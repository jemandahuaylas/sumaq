...
export const SecundariaTemplate: React.FC<TemplateProps> = ({ config, student }) => {
    return (
        <div className="w-full h-full relative bg-[#FDFBF7] overflow-hidden text-slate-900 font-serif">
            {/* Marco Doble Elegante */}
            <div className="absolute inset-4 border border-slate-800 z-20"></div>
            <div className="absolute inset-6 border border-yellow-600 z-20"></div>
            ...
            {/* Adornos Esquinas */}
            <div className="absolute top-4 left-4 w-20 h-20 border-t-4 border-l-4 border-slate-900 z-30"></div>
            <div className="absolute bottom-4 right-4 w-20 h-20 border-b-4 border-r-4 border-slate-900 z-30"></div>
            ...
            <div className="relative z-10 flex flex-col items-center justify-between h-full py-8 px-24">
                ...
                <div className="text-center mb-6">
                    ...
                    <h2 className="text-lg font-medium tracking-[0.2em] text-slate-600 uppercase border-b border-slate-300 pb-1 inline-block">
                ...
                <div className="text-center mb-6">
                    <h1 className="text-5xl font-serif tracking-widest text-slate-900 mb-1 font-medium">
                        {config.tituloDiploma}
                    </h1>
                    <div className="h-1 w-20 bg-gradient-to-r from-transparent via-yellow-600 to-transparent mx-auto"></div>
                </div>
                ...
                <div className="text-center w-full flex-1 flex flex-col justify-center pb-2">
                    ...
                    <div className="text-4xl font-serif text-slate-900 mb-6 py-2 border-y border-slate-100 italic capitalize">
                        {student.nombres}
                    </div>
                    ...
                    <div className="w-full flex justify-center gap-12">
                        {config.firmas.map((signer) => (
                            <div key={signer.id} className="text-center w-40">
                                ...
                                <div className="h-px w-full bg-slate-900 mb-1"></div>
                                <p className="font-serif font-bold text-xs tracking-wide">{signer.nombre}</p>
