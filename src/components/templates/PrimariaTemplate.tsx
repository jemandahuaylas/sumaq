...
export const PrimariaTemplate: React.FC<TemplateProps> = ({ config, student }) => {
    return (
        <div className="w-full h-full relative bg-white overflow-hidden text-slate-800">
            ...
            <svg className="absolute inset-0 w-full h-full opacity-10 z-0" xmlns="http://www.w3.org/2000/svg">
                <defs><pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse"><path d="M 40 0 L 0 0 0 40" fill="none" stroke="#2563EB" strokeWidth="1" /></pattern></defs>
                <rect width="100%" height="100%" fill="url(#grid)" />
            </svg>

            {/* Bandas Laterales */}
            <div className="absolute left-0 top-0 bottom-0 w-4 bg-blue-600 z-20"></div>
            <div className="absolute right-0 top-0 bottom-0 w-4 bg-teal-500 z-20"></div>

            {/* Esquina Decorativa */}
            <div className="absolute top-0 right-0 w-0 h-0 border-t-[120px] border-l-[120px] border-t-teal-500/20 border-l-transparent z-10"></div>
            <div className="absolute bottom-0 left-0 w-0 h-0 border-b-[120px] border-r-[120px] border-b-blue-600/20 border-r-transparent z-10"></div>
            ...
            <div className="relative z-10 flex flex-col justify-between h-full py-6 px-20">
                ...
                <div className="flex items-center justify-between border-b-2 border-slate-100 pb-4">
                    ...
                    <div className="text-right">
                        ...
                        <span className="text-xs font-bold bg-blue-100 text-blue-700 px-3 py-1 rounded uppercase tracking-widest">Excelencia Educativa</span>
                    </div>
                </div>
                ...
                <div className="text-center py-4 flex-1 flex flex-col justify-center pb-2">
                    ...
                    <div className="relative inline-block mb-6">
                        <div className="absolute inset-0 bg-blue-200 blur-xl opacity-30 transform -skew-x-12"></div>
                        <h2 className="relative text-5xl font-black text-blue-900 uppercase transform -skew-x-6 z-10" style={{ fontFamily: 'Impact, sans-serif' }}>
                            {student.nombres}
                        </h2>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-20 border-t-2 border-slate-100 pt-4">
                        {config.firmas.slice(0, 2).map((signer) => (
                            <div key={signer.id} className="text-center">
                                ...
                                <p className="text-[10px] text-blue-600 font-bold uppercase">{signer.cargo}</p>
                            </div>
                        ))}
                    </div>
                </div>
